...
navigator.clipboard.writeText(text)
  ...
  const showPassword = () =>{
    ...
    if(  ref.current.src.includes("public/closed-eyes.png")){
    ref.current.src = "public/eye.png"
      passwordRef.current.type = "password"
    }
  else{
    ref.current.src = "public/closed-eyes.png"
      passwordRef.current.type = "text"
  }
}
  const savePassword = async () => {
      if(form.site.length>3 && form.username.length > 3 && form.password.length > 3){
           ...
           await fetch("http://localhost:3000/", {method: "DELETE", hadders: {"content-type": "application/json" },
        body: JSON.stringify({id: form.id}) })


        //if any such id exists in the db, delete it
        setPasswordArray([...passwordArray, {...form, id: uuidv4()}])
       await fetch("http://localhost:3000/", {method: "post", hadders: {"content-type": "application/json" },
   ...
   let res = await fetch("http://localhost:3000/", {method: "DELETE", hadders: {"content-type": "application/json" },
...
<div className="p-2 pt-3 md:px-30 md:py-16 max-w-7xl mx-auto min-h-[88.2vh]">
        ...
        <div className=" flex flex-col  p-4 text-black items-center  gap-8">
          ...
          <span className="absolute right-[3px] top-[3px] cursor-pointer" onClick={showPassword}>
            <img ref={ref} className="p-1" width={28} src="public/eye.png" alt="eye" /></span>
          </div>
          </div>
        ...
        {passwordArray.length === 0 && <div> No passowrd  to show</div> }
